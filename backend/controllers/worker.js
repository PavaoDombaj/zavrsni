...
export const createWorker = async (req, res, next) => {
    try {
        ...
        console.log(newWorkerSalon.id)
        // Stvori novog radnika s istim ID-om korisnika
        const newWorker = new Worker({
            name: req.body.name,
            role: req.body.role,
            user: existingUser.id,
            _id: existingUser.id, // Postavi user polje na ID postojećeg korisnika
            salons: newWorkerSalon,
            workSchedules: [],
        });
        // Ažuriraj polje salons u tablici User
        existingUser.salons.push(newWorkerSalon.id); ///existing user = id korisnika kod kojege se u salons.   
        //pusha vrijednost njegovog salona u kojem radi
        await existingUser.save();

        /// Ažuriraj polje workers u tablici Salon
        newWorkerSalon.workers.push(existingUser.id)
        await newWorkerSalon.save();

        console.log("newWorkerSalon._id: " + newWorkerSalon.id); //test
